  ...
  >
    <div class="max-w-4xl mx-auto">
      ...
      <div class="text-center mb-8 sm:mb-10">
        ...
        <p class="font-body text-sm sm:text-base text-charcoal-light dark:text-dark-text-secondary max-w-2xl mx-auto">
          {{ t.wishlist?.subtitle || "If you'd like to bless us with a gift, here are some ideas. Your presence is the greatest gift!" }}
        </p>
      </div>

      <!-- Loading -->
      <div v-if="isLoading" class="flex justify-center py-12">
        <div class="w-8 h-8 border-2 border-sage border-t-transparent rounded-full animate-spin"></div>
      </div>
      ...
      <div v-else>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          >
            ...
            <div class="relative aspect-video bg-sand-dark dark:bg-dark-bg">
              ...
              >
                <span class="px-3 py-1.5 bg-charcoal/80 text-white text-xs sm:text-sm font-body rounded-full">
            ...
            <div class="p-4 flex-1 flex flex-col">
              ...
              <p class="font-body text-xs sm:text-sm text-charcoal-light dark:text-dark-text-secondary mb-2 line-clamp-2">
                {{ getLocalizedText(gift.description) }}
              </p>

              <!-- Notes -->
              <p v-if="gift.notes" class="font-body text-xs text-sage-dark dark:text-sage-light italic mb-2">
                {{ gift.notes }}
              </p>
              ...
              <div class="mt-auto flex gap-2">
                ...
                >
                  {{ isGiftAvailable(gift) ? (t.wishlist?.reserveButton || "Reserve") : (t.wishlist?.fullyReserved || "Reserved") }}
                </button>
              </div>
            </div>
          </div>
        ...
        <!-- Footer Note -->
        <p class="mt-8 text-center font-body text-xs sm:text-sm text-charcoal-light dark:text-dark-text-secondary italic">
          {{ t.wishlist?.footerNote || "Feel free to give any gift from the heart - this list is just to help avoid duplicates!" }}
        </p>
      </div>
    </div>
    ...
    <Teleport to="body">
      <Transition name="modal">
        ...
        >
          ...
          <!-- Modal Content -->
          <div class="relative w-full max-w-md bg-white dark:bg-dark-bg-secondary rounded-2xl shadow-xl p-6 max-h-[90vh] overflow-y-auto">
            <!-- Success State -->
            <div v-if="reserveSuccess" class="text-center py-6">
              <div class="w-16 h-16 mx-auto mb-4 flex items-center justify-center text-3xl text-white bg-sage rounded-full">
                ✓
              </div>
            ...
            <div v-else>
              ...
              <!-- Error -->
              <div v-if="reserveError" class="mb-4 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
                <p class="text-sm text-red-600 dark:text-red-400">{{ reserveError }}</p>
              </div>
              ...
              <form @submit.prevent="handleSubmit" class="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows="2"
                    :placeholder="t.wishlist?.messagePlaceholder || 'Leave a message for the couple...'"
                ...
                <div class="flex gap-3 pt-2">
                  ...
                  <button
                    type="submit"
                    :disabled="isReserving || !formData.guestName.trim() || !formData.guestPhone.trim()"
                  ...
                  >
                    {{ isReserving ? (t.wishlist?.submitting || "Submitting...") : (t.wishlist?.submitReserve || "Reserve Now") }}
                  </button>
                </div>
              </form>
            </div>

...
<template>
  <div ref="dropdownRef" class="relative">
    ...
    >
      <div class="w-8 h-8 rounded-full bg-sage flex items-center justify-center text-white font-body text-sm font-medium">
        {{ getInitial(username) }}
      </div>
      <span class="font-body text-sm text-charcoal dark:text-dark-text hidden sm:inline">
        {{ username }}
      </span>
      <span v-if="isMasterUser" class="hidden sm:inline px-1.5 py-0.5 text-xs bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 rounded-full">
        Master
      </span>
      <svg

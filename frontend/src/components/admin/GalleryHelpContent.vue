...
<template>
  <div class="space-y-4 font-body text-sm">
    ...
    <div class="flex gap-3">
      <div class="flex-shrink-0 w-6 h-6 rounded-full bg-sage/20 dark:bg-sage/30 flex items-center justify-center">
        <svg class="w-3.5 h-3.5 text-sage-dark dark:text-sage-light" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <div class="flex gap-3">
      ...
      <div>
        ...
        <p class="text-charcoal-light dark:text-dark-text-secondary mt-0.5">
          Formats: {{ allowedFormats.map(f => formatLabels[f] ?? f).join(', ') }}
        </p>
      </div>
    </div>

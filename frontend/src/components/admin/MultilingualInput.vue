...
<template>
  <div class="space-y-3">
    <p class="font-body text-sm font-medium text-charcoal dark:text-dark-text">{{ label }}</p>
    <div v-for="lang in languages" :key="lang.key">
      ...
      <input
        ...
        :placeholder="placeholder ? `${placeholder} (${lang.label})` : `${lang.label} ${label.toLowerCase()}`"
        :required="requiredLanguages?.includes(lang.key)"
        @input="updateField(lang.key, ($event.target as HTMLInputElement).value)"
      />
    </div>
  </div>
</template>

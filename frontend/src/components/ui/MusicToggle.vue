      ...
      <div class="flex items-center gap-1">
        ...
        >
          <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" :d="showControls ? 'M5 15l7-7 7 7' : 'M19 9l-7 7-7-7'" />
          </svg>
        </button>
      </div>
    ...
    <Transition name="controls">
      ...
      >
        ...
        <div class="flex items-center gap-2 mb-3">
          ...
          />
          <span class="text-white/70 text-xs w-8 text-right">{{ Math.round(localVolume * 100) }}%</span>
        </div>
        ...
        <div v-if="currentTrack" class="text-white text-xs mb-2">
          ...
          <p class="text-white/60 truncate">
            {{ currentTrack.artist || "Unknown" }} · {{ formatDuration(currentTrack.duration) }}
          </p>
        </div>

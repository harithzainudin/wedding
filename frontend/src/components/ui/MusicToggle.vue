...
const currentTrack = computed((): MusicTrack | null => {
  ...
  if (playMode.value === "single" && settings.value.selectedTrackId) {
    return (
      sortedTracks.value.find(
        (t) => t.id === settings.value!.selectedTrackId,
      ) ?? null
    );
  }
...
const hasNextTrack = computed(() => {
  ...
  return (
    currentTrackIndex.value < sortedTracks.value.length - 1 || shouldLoop.value
  );
});
  ...
  try {
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      if (audioElement.value) {
        audioElement.value.volume = Math.max(
          0,
          localVolume.value - volumeStep * i,
        );
      }
      if (nextAudioElement.value) {
        nextAudioElement.value.volume = Math.min(
          localVolume.value,
          volumeStep * i,
        );
      }
    ...
    >
      <div class="flex items-center gap-1">
        ...
        >
          ...
          <div
            v-if="isPlaying"
            class="w-8 h-1 bg-white/30 rounded-full overflow-hidden"
          >
            <div
          ...
          <span
            class="font-body text-[10px] sm:text-xs font-medium uppercase tracking-wider"
          >
            {{ isPlaying ? "ON" : "OFF" }}
          </span>
        </button>
      ...
      <Transition name="tooltip">
        ...
        >
          ...
          {{ currentTrack.title }}
          <span v-if="currentTrack.artist" class="text-white/60"

<script setup lang="ts">
   ...
   *
   * A FAB (Floating Action Button) that expands to show navigation actions.
   * Used in layouts that don't have a sticky footer (storybook, page-slideshow).
   */
  ...
  // Navigation popup state (for maps bottom sheet)
...
<template>
  ...
  <Teleport to="body">
    <Transition name="fade">
      <div v-if="isExpanded" class="fixed inset-0 bg-black/30 z-[49]" @click="closeFab" />
    </Transition>
  </Teleport>

  <!-- FAB Container - Stop touch events from bubbling to slideshow swipe handlers -->
  <div
    class="fixed bottom-20 right-6 z-50 flex flex-col items-end gap-3"
    @touchstart.stop
    @touchmove.stop
    @touchend.stop
  >
    ...
    <Transition name="fab-expand">
      <div v-if="isExpanded" class="flex flex-col items-end gap-2">
        ...
        <button
          v-if="showRsvpButton"
          type="button"
          class="flex items-center gap-3 px-4 py-3 bg-white dark:bg-dark-bg-secondary rounded-full shadow-md text-charcoal dark:text-dark-text font-body text-sm font-medium transition-all duration-200 hover:shadow-lg active:scale-95 cursor-pointer min-h-[44px]"
          @click="scrollToRsvp"
        >
          ...
          <span class="whitespace-nowrap">RSVP</span>
        </button>
        ...
        >
          ...
          <span class="whitespace-nowrap">{{ t.nav.gifts || 'Gifts' }}</span>
        </button>
        ...
        >
          ...
          <span class="whitespace-nowrap">{{ t.nav.calendar }}</span>
        </button>
        ...
        >
          ...
          <span class="whitespace-nowrap">{{ t.nav.navigate || 'Navigate' }}</span>
        </button>
      </div>
    </Transition>
    ...
    <button
      type="button"
      class="w-12 h-12 rounded-full bg-sage text-white shadow-lg flex items-center justify-center transition-all duration-300 hover:bg-sage-dark active:scale-95 cursor-pointer"
      :class="{ 'rotate-90': isExpanded }"
      aria-label="Toggle menu"
      @click="toggleFab"
    >
      <!-- Menu icon (hamburger) when collapsed, X when expanded -->
...
<style scoped>
  ...
  .fab-expand-enter-active {
    animation: fab-expand-in 0.3s ease-out;
  }

  .fab-expand-leave-active {
    animation: fab-expand-out 0.2s ease-in;
  }

  @keyframes fab-expand-in {
    0% {
      opacity: 0;

...
export const VALID_CATEGORIES: GiftCategory[] = ["home", "kitchen", "electronics", "experiences", "other"];
...
export function validateCreateGiftInput(input: unknown): {
  ...
  if (!isValidMultilingualText(body.name)) {
    return { valid: false, error: "Name is required in Malay (ms) and English (en)" };
  }
  const nameInput = body.name as Record<string, string>;
  if (nameInput.en.length > GIFT_LIMITS.maxNameLength) {
    return { valid: false, error: `Name must be less than ${GIFT_LIMITS.maxNameLength} characters` };
  }
  ...
  if (!isValidMultilingualText(body.description)) {
    return { valid: false, error: "Description is required in Malay (ms) and English (en)" };
  }
  const descInput = body.description as Record<string, string>;
  if (descInput.en.length > GIFT_LIMITS.maxDescriptionLength) {
    return { valid: false, error: `Description must be less than ${GIFT_LIMITS.maxDescriptionLength} characters` };
  }
  ...
  if (body.externalLink !== undefined && body.externalLink !== null && body.externalLink !== "") {
    ...
    if (body.externalLink.length > GIFT_LIMITS.maxExternalLinkLength) {
      return { valid: false, error: `External link must be less than ${GIFT_LIMITS.maxExternalLinkLength} characters` };
    }
  ...
  if (body.priceRange.length > GIFT_LIMITS.maxPriceRangeLength) {
    return { valid: false, error: `Price range must be less than ${GIFT_LIMITS.maxPriceRangeLength} characters` };
  }
  ...
  if (body.notes !== undefined && body.notes !== null && body.notes !== "") {
    ...
    if (body.notes.length > GIFT_LIMITS.maxNotesLength) {
      return { valid: false, error: `Notes must be less than ${GIFT_LIMITS.maxNotesLength} characters` };
    }
  }
  ...
  if (isNaN(qty) || qty < 1 || qty > GIFT_LIMITS.maxQuantityPerItem) {
    return { valid: false, error: `Quantity must be between 1 and ${GIFT_LIMITS.maxQuantityPerItem}` };
  }
...
export function validateUpdateGiftInput(input: unknown): {
  ...
  if (body.name !== undefined) {
    if (!isValidMultilingualText(body.name)) {
      return { valid: false, error: "Name must be provided in Malay (ms) and English (en)" };
    }
    const nameInput = body.name as Record<string, string>;
    if (nameInput.en.length > GIFT_LIMITS.maxNameLength) {
      return { valid: false, error: `Name must be less than ${GIFT_LIMITS.maxNameLength} characters` };
  ...
  if (body.description !== undefined) {
    if (!isValidMultilingualText(body.description)) {
      return { valid: false, error: "Description must be provided in Malay (ms) and English (en)" };
    }
    const descInput = body.description as Record<string, string>;
    if (descInput.en.length > GIFT_LIMITS.maxDescriptionLength) {
      return { valid: false, error: `Description must be less than ${GIFT_LIMITS.maxDescriptionLength} characters` };
  ...
  if (body.externalLink !== undefined) {
    if (typeof body.externalLink !== "string" || !body.externalLink.trim()) {
      return { valid: false, error: "External link cannot be empty" };
    }
    if (body.externalLink.length > GIFT_LIMITS.maxExternalLinkLength) {
      return { valid: false, error: `External link must be less than ${GIFT_LIMITS.maxExternalLinkLength} characters` };
    }
    if (!isValidUrl(body.externalLink)) {
      return { valid: false, error: "External link must be a valid URL" };
    }
...
export function validateReserveGiftInput(input: unknown): {
  ...
  if (typeof body.guestName !== "string" || body.guestName.trim().length < 2) {
    return { valid: false, error: "Guest name is required and must be at least 2 characters" };
  }
  ...
  if (body.message !== undefined && body.message !== null && body.message !== "") {
    ...
    if (body.message.length > GIFT_LIMITS.maxReservationMessageLength) {
      return { valid: false, error: `Message must be less than ${GIFT_LIMITS.maxReservationMessageLength} characters` };
    }
  }
  ...
  return {
    ...
    data: {
      ...
      rsvpId: typeof body.rsvpId === "string" && body.rsvpId.trim() ? body.rsvpId.trim() : undefined,
      quantity,
...
export function validateGiftSettingsInput(input: unknown): {
  ...
  if (body.maxFileSize !== undefined) {
    ...
    if (isNaN(maxFileSize) || maxFileSize < 1024 || maxFileSize > 10 * 1024 * 1024) {
      return { valid: false, error: "Max file size must be between 1KB and 10MB" };
    }
  ...
  if (body.allowedFormats !== undefined) {
    ...
    for (const format of body.allowedFormats) {
      if (typeof format !== "string" || !validFormats.includes(format)) {
        return { valid: false, error: `Invalid format: ${format}. Valid formats: ${validFormats.join(", ")}` };
      }
    }

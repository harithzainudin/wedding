...
function normalizeMultilingualText(
...
export function validateUpdateGiftInput(input: unknown):
  ...
  if (body.notes !== undefined) {
    ...
    if (
      typeof body.notes === "string" &&
      body.notes.length > GIFT_LIMITS.maxNotesLength
    ) {
      return {
        valid: false,
        error: `Notes must be less than ${GIFT_LIMITS.maxNotesLength} characters`,
...
export function validateReserveGiftInput(input: unknown):
  ...
  if (
    isNaN(quantity) ||
    quantity < 1 ||
    quantity > GIFT_LIMITS.maxQuantityPerItem
  ) {
    return {
      valid: false,
      error: `Quantity must be between 1 and ${GIFT_LIMITS.maxQuantityPerItem}`,
  ...
  return {
    ...
    data: {
      ...
      rsvpId:
        typeof body.rsvpId === "string" && body.rsvpId.trim()
          ? body.rsvpId.trim()
          : undefined,
      quantity,
      message:

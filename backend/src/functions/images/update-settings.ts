...
export const handler: APIGatewayProxyHandlerV2 = async (event, context) => {
  try {
    ...
    const newSettings = {
      ...
      sk: "IMAGES",
      maxFileSize:
        validation.data.maxFileSize ??
        currentResult.Item?.maxFileSize ??
        DEFAULT_MAX_FILE_SIZE,
      maxImages:
        validation.data.maxImages ??
        currentResult.Item?.maxImages ??
        DEFAULT_MAX_IMAGES,
      allowedFormats: currentResult.Item?.allowedFormats ?? [
        ...ALLOWED_MIME_TYPES,
      ],
      showGallery:
        validation.data.showGallery ??
        currentResult.Item?.showGallery ??
        DEFAULT_SHOW_GALLERY,
      updatedAt: now,

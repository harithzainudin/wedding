...
import { validateRefreshToken, generateAccessToken, generateRefreshToken } from '../shared/auth'
...
export const handler: APIGatewayProxyHandlerV2 = async (event, context) => {
  try {
    ...
    const newAccessToken = generateAccessToken(result.user.username, result.user.isMaster)
    const newRefreshToken = generateRefreshToken(result.user.username, result.user.isMaster)

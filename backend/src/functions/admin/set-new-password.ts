...
export const handler: APIGatewayProxyHandlerV2 = async (event, context) => {
  ...
  try {
    ...
    if (!result.Item.mustChangePassword) {
      return createErrorResponse(
        403,
        'This endpoint is only for users who must change their password. Use the regular password change endpoint instead.',
        context,
        'FORBIDDEN'
      )
    }
    ...
    await docClient.send(
      new UpdateCommand({
        ...
        UpdateExpression:
          'SET passwordHash = :newHash, mustChangePassword = :must, updatedAt = :updatedAt',
    ...
    logError(
      {
        endpoint: 'PUT /admin/users/set-password',

...
export const handler: APIGatewayProxyHandlerV2 = async (event, context) => {
  ...
  try {
    ...
    if (!body.username || body.username.trim().length < 3) {
      return createErrorResponse(400, "Username must be at least 3 characters", context, "VALIDATION_ERROR");
    }

    if (!body.password || body.password.length < 6) {
      return createErrorResponse(400, "Password must be at least 6 characters", context, "VALIDATION_ERROR");
    }
